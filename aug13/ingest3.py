...
# Target chunk sizes (characters); packer will merge adjacent content to reach ~target
TARGET_CHARS = 900
MAX_CHARS = 1400
MIN_CHARS = 400

# Header/footer detection
TOP_MARGIN_PCT = 0.09     # top 9% of page height
BOTTOM_MARGIN_PCT = 0.09  # bottom 9% of page height
BANNER_FREQ_THRESHOLD = 0.30  # if a normalized line appears in >=30% of pages -> treat as banner

# Regex patterns to drop anywhere (even if not in margin)
BAN_PATTERNS = [
    r"\b(?:Export(?:\s+and)?\s+Distribution\s+Controlled)\b.*",     # export notices
...
# Date-like lines to drop when likely header/footer
...
def normalized_banner_key(text: str) -> str:
    # aggressive normalization: collapse spaces, drop page numbers
    t = norm_ws(text)
...
def collect_banners(lines_by_page: Dict[int, List[Line]], page_heights: Dict[int, float]) -> Dict[str, str]:
    """
    Returns a dict of normalized banner text -> 'top' | 'bottom' likely banners.
    """
...
def is_banner_line(ln: Line, page_height: float, banners: Dict[str, str]) -> bool:
    ...
    if key in banners:
        if (banners[key] == "top" and ln.y0 <= top_y) or (banners[key] == "bottom" and ln.y0 >= bot_y):
            return True
    # Rule-based quick drop (even mid-page)
...
def pack_chunks(typed_lines: List[TypedLine]) -> List[Tuple[BlockType, str, str, str]]:
    """
    Merge adjacent content into ~TARGET_CHARS chunks, but never cross section boundaries.
    Returns list of (bt, text, section_path, section_label)
    """
    ...
    for tl in typed_lines:
        if tl.bt == "table":
            # tables are emitted separately by caller; just flush any buffer
            flush()
            continue

        # new section boundary -> flush
        ...
        # keep merging similar content types (paragraph/list/code)
        if tl.bt == buf_bt and (buf_len < TARGET_CHARS or (buf_len < MAX_CHARS and not tl.text.endswith((".", ":", ";")))):
            ...
            # if we exceeded MAX_CHARS, flush now
            if buf_len >= MAX_CHARS:
                flush()
            continue

        # type changed or buffer big enough -> flush then start new
        flush()
        buf_bt = tl.bt
        buf_sec_path = tl.section_path
        buf_sec_label = tl.section_label
    ...
    flush()
    # enforce minimum size by gluing tiny trailing chunks (within same section)
...
def ingest_pdf_to_jsonl(pdf_path: Path, out_jsonl: Path, tables_dir: Path) -> None:
    ...
    # Pass A: read pages, collect lines & heights, detect TOC
    lines_by_page: Dict[int, List[Line]] = {}
    ...
    for p in tqdm(range(len(doc)), desc=f"[{doc_id}] Pass A: read pages", leave=False):
        page_idx0 = p
        lines, height = extract_lines(doc, page_idx0)
        page_no = page_idx0 + 1
    ...
    # Pass A2: detect repeating banners (headers/footers)
    banners = collect_banners(lines_by_page, page_heights)

    # Heading candidates (outline + text, excluding TOC pages)
    ...
    for page_no, lines in tqdm(lines_by_page.items(), desc=f"[{doc_id}] Pass A: heading candidates", leave=False):
        if tocs.get(page_no, False):
            continue
        # Drop banner lines before heading detection to avoid false positives
    ...
    with out_jsonl.open("a", encoding="utf-8") as fo:
        for p in tqdm(range(len(doc)), desc=f"[{doc_id}] Pass B: emit", leave=False):
            ...
            # Filter lines: drop banners + hard-banned patterns
            flines: List[Line] = []
            for ln in lines:
                if is_banner_line(ln, page_heights[page_no], banners):
                    continue
                # also drop empty or tiny boilerplate
                t = ln.text.strip()
                if not t:
                    continue
                flines.append(ln)
            ...
            # Capture section intros: first paragraph we see per section_path
            for tl in typed_lines:
            ...
            # Pack into bigger chunks (tables handled separately below)
            packed = pack_chunks(typed_lines)

            # Build context (before/after) across packed chunks on this page
            ...
            for i, (bt, text, sec_path, sec_label) in enumerate(packed):
                ...
                rec = BlockRecord(
                    ...
                    span_end=span_end,
                    ts_extracted=time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                    source_path=str(pdf_path),
                )
                fo.write(json.dumps(asdict(rec), ensure_ascii=False) + "\n")

            # Emit tables under the last section of the page
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Accurate PDF ingestion (healed sections, banner filtering, larger chunks).")
