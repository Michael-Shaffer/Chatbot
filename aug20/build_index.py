...
# Fixed to work with your ingest.py and retriever.py
...
from ingest import ingest_pdf_to_jsonl
...
def normalize_chunk_fields(chunk: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ingest.py uses 'block_id' while retriever.py expects 'chunk_id'
    """
    # Create a new dict with renamed fields
    normalized = chunk.copy()
    
    # Rename block_id to chunk_id
    if 'block_id' in normalized:
        normalized['chunk_id'] = normalized['block_id']
        del normalized['block_id']
...
def build_index(
    ...
    # Process each PDF using ingest_pdf_to_jsonl
    temp_jsonl = outdir / "temp_chunks.jsonl"
    ...
    for pdf in pdfs:
        print(f"[ingest] {pdf}")
        # Call the actual ingest function from ingest.py
        ingest_pdf_to_jsonl(
            pdf_path=pdf,
            out_jsonl=temp_jsonl,
...
def load_retriever(index_dir: Union[str, Path]) -> Retriever:
    ...
    # Normalize fields before fitting
    items = [normalize_chunk_fields(chunk) for chunk in items]
    r = Retriever()
    r.fit(items)
    return r
