...
def main() -> None:
    ...
    for b in tqdm(blocks, desc="Preparing texts"):
        ...
        # IMPORTANT CHANGE: Save the ENTIRE chunk, not just selected fields
        # This ensures all fields from ingest.py are available at retrieval time
        meta.append(b)  # <-- THIS IS THE KEY CHANGE
    ...
    print(f"   - Chunks indexed: {len(blocks)}")
    print(f"   - Embedding dimension: {vecs.shape[1] if vecs.size else 0}")
