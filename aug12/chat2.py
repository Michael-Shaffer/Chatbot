...
def detect_heading_indices(lines: Sequence[str]) -> set[int]:
    return {i for i, t in enumerate(lines) if len(t) <= 100 and _HEADING_RE.match(t)}
...
def extract_sorted_lines(page: fitz.Page) -> List[str]:
    ...
    return [norm_ws(b[4]) for b in blocks if norm_ws(b[4])]


def build_typed_blocks(lines: Sequence[str]) -> List[Tuple[BlockType, str]]:
    headings = detect_heading_indices(lines)
    return [(classify_line(t, is_heading=(i in headings)), t) for i, t in enumerate(lines)]
...
def iter_page_records(
    ...
    for bi, (bt, text) in enumerate(typed_blocks):
        ...
        records.append(BlockRecord(
            doc_id, page_no, f"{doc_id}_p{page_no:03d}_b{bi:03d}",
            bt, section_path, html, text, text,
            None, None, None,
    ...
    for tbl in table_items:
        ...
        offset = span_end
        records.append(BlockRecord(
            doc_id, page_no, tbl.table_id,
            "table", section_path, tbl.markdown, tbl.markdown, tbl.markdown,
            tbl.table_id, tbl.markdown, str(tbl.csv_path),
            "", "",
            span_start, span_end,
            ts_iso, str(pdf_path)
        ))
    ...
    with out_jsonl.open("a", encoding="utf-8") as fo:
        for p in tqdm(range(len(doc)), desc=f"[{doc_id}] Extracting pages", leave=False):
            ...
            records, offset = iter_page_records(
                pdf_path, doc_id, page, page_no, section_stack, offset, ts_iso, tables_dir
            )
            for r in records:
